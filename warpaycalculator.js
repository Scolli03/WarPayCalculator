...
(function() {
    console.log('War Payment Calculator loaded');
    ...
    PDA_httpGet(`https://api.torn.com/faction/?selections=basic&key=$${api_key}`).then(response => {
        const factionData = JSON.parse(response);
        player_faction = factionData.name;
        leader_id = factionData.leader_id;
        co_leader_id = factionData.co_leader_id;
        console.log('Faction: ' + player_faction);
        console.log('Leader ID: ' + leader_id);
        console.log('Co-Leader ID: ' + co_leader_id);
    });    

    let selector = '.your-faction';
    let list_item = 'li.your ';

    let winner = document.querySelector('.t-blue').textContent;
